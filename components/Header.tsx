...
const Header: React.FC<HeaderProps> = ({ setActivePath, theme, setTheme }) => { // Removed activePath as it's no longer used for navigation rendering
  ...
  return (
    <header className="bg-white dark:bg-gray-800 shadow-sm dark:shadow-md dark:shadow-black/20 p-4 flex items-center justify-between">
      {/* POS System Title (Right in RTL) */}
      <div className="flex items-center">
        <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">{AR_LABELS.posSystem}</h2>
      </div>
      ...
      <div className="flex items-center space-x-4 space-x-reverse">
        ...
        <div className="flex items-center space-x-2 space-x-reverse">
          <span className="text-gray-700 dark:text-gray-300">{AR_LABELS.ahmadSai}</span>
          <img src="https://picsum.photos/40/40?random=5" alt="User Avatar" className="w-8 h-8 rounded-full" />
        </div>
      </div>
    </header>
  );
