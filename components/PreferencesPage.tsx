...
const PreferencesPage: React.FC = () => {
  ...
  const renderField = (label: string, name: keyof SystemPreferences, type: 'text' | 'number' | 'textarea', children?: React.ReactNode) => (
    <div className="grid grid-cols-3 items-center text-right">
        ...
        <div className="col-span-2">
            {type === 'textarea' ? (
                <textarea id={name} name={name} value={prefs[name] as string} onChange={handleChange} rows={3} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm text-right"/>
            ) : (
                <input type={type} id={name} name={name} value={prefs[name] as string | number} onChange={handleChange} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm text-right"/>
            )}
             {children}
        </div>
    </div>
   ...
   const renderSelect = (label: string, name: keyof SystemPreferences, options: {value: string, label: string}[]) => (
     <div className="grid grid-cols-3 items-center text-right">
        <label htmlFor={name} className="col-span-1 text-sm font-medium text-gray-700 dark:text-gray-300">{label}</label>
        <select id={name} name={name} value={prefs[name] as string} onChange={handleChange} className="col-span-2 w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm text-right">
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{AR_LABELS.preferences}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {renderSection(AR_LABELS.generalSettings, <>
                ...
                <div className="grid grid-cols-3 items-center text-right">
                    ...
                    <div className="col-span-2 flex items-center gap-4">
                        {logoPreview && <img src={logoPreview} alt="logo preview" className="h-12 w-12 object-cover rounded-md"/>}
                        <input type="file" onChange={handleLogoChange} accept="image/*" className="text-sm text-gray-700 dark:text-gray-300"/>
                    </div>
                </div>
       ...
       <div className="flex justify-start mt-6">
            <button
                ...
                className="px-6 py-2 bg-orange-500 text-white font-semibold rounded-md shadow-sm hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500"
            >
